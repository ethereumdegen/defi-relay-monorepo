...
use crate::models::{
    usdc_address, DomainEthAddress, DomainUint256, PaymentPayload, PaymentRequired,
    PaymentResponse, VerifyPaymentRequirements, BASE_NETWORK,
};
...
impl X402Middleware {
    pub fn new(
        ...
        X402Middleware {
            ...
            description: "EVM RPC access".to_string(),
            facilitator,
            nonce_tracker,
        }
    }
            ...
            match payment_header {
                ...
                Some(payment_header_value) => {
                    ...
                    if !nonce_tracker.try_use_nonce(&nonce_hex) {
                        ...
                        let payment_response = PaymentResponse::failure(
                    ...
                    match verify_result {
                        Ok(verify_response) if verify_response.is_valid => {
                            info!(
                                "Payment verified, attempting settlement before processing request"
                            );
                            ...
                            match settle_result {
                                ...
                                Ok(settle_response) => {
                                    ...
                                    error!(
                                        "Payment settlement failed: {}. Request rejected.",
                                        error_msg
                                    );
                                ...
                                Err(e) => {
                                    ...
                                    let payment_response =
                                        PaymentResponse::failure(&format!("Settlement error: {}", e));
                                    let encoded = payment_response.to_base64().unwrap_or_default();
