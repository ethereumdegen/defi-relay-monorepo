...
impl NonceTracker {
    ...
    ///
    /// The TTL should be longer than the max_timeout_seconds in payment requirements
    /// to ensure nonces aren't evicted before they could be replayed.
    pub fn new(ttl: Duration) -> Self {
        let cache = Cache::builder()
            .time_to_live(ttl)
            .max_capacity(100_000) // Limit memory usage
            .build();
    ...
    pub fn try_use_nonce(&self, nonce: &str) -> bool {
        ...
        // Try to insert - use get_with to handle race conditions
        // If another thread inserted between our check and insert, this still works
...
mod tests {
    ...
    fn test_nonce_replay_fails() {
        let tracker = NonceTracker::with_default_ttl();
        assert!(tracker.try_use_nonce("nonce1"));
        assert!(!tracker.try_use_nonce("nonce1")); // Replay should fail
    }
